...
f'#SBATCH --output=scripts/ex3rendering_speed/sbatch_logs/{job_name}/%A_%a.log',
'''#SBATCH --mem=64G
#SBATCH --time=3:00:00
...
''',
        ...
        'CUDA_VISIBLE_DEVICES=0 python train.py -s  ../../../data/GS/${dataset}/${scene} --eval --lod 0 --voxel_size 0.001 --update_init_factor 16 --iterations 30_000 -m outputs/ex3rendering_speed/${dataset}/${scene}/${lmbda} --lmbda ${lmbda} --mask_lr_final ${mask_lr_final} --save_iterations 1000 30000',
        
    ]
    script = '\n'.join(scripts)
    save_path = Path(f'scripts/ex3rendering_speed/tmp/submit.sh')
...
if __name__ == '__main__':
    ...
    # mask_lr_final = 0.0005 * lmbda / 0.001
    # mask_lr_final = min(mask_lr_final, 0.0015)
    submit_job(lmbda = lmbda, run_script = True, job_name = 'HACplus_ex3rendering_speed')