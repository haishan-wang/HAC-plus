...
f'#SBATCH --output=scripts/ex2test_param/sbatch_logs/{job_name}/%A_%a.log',
'''#SBATCH --mem=64G
#SBATCH --time=3:00:00
...
test_iteration=$(seq 0 100 30000)
''',
        f'lmbda={lmbda}',
        f'mask_lr_final={mask_lr_final}',
        'CUDA_VISIBLE_DEVICES=0 python train.py -s  ../../../data/GS/${dataset}/${scene} --eval --lod 0 --voxel_size 0.005 --update_init_factor 16 --iterations 30_000 -m outputs/ex1reproduce/${dataset}/${scene}/${lmbda} --lmbda ${lmbda} --mask_lr_final ${mask_lr_final} --use_wandb --test_iterations ${test_iteration}',
        
    ]
    script = '\n'.join(scripts)
    save_path = Path(f'scripts/ex2test_param/tmp/submit.sh')
